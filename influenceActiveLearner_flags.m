...
fId5 = fopen(strcat(Folder2, 'influenceActiveLearnerBatchReliability_', DatasetName, '_20.txt'), 'w');
fId6 = fopen(strcat(Folder2, 'influenceActiveLearnerBatchKappaConsensus_', DatasetName, '_20.txt'), 'w');
fId7 = fopen(strcat(Folder2, 'influenceActiveLearnerBatchKappatrueLabels_', DatasetName, '_20.txt'), 'w');
fprintf(fId, '\n-----------------------------------------------------\n');
fprintf(fId2, '\n-----------------------------------------------------\n');
fprintf(fId, '\nDataset is %s\n',DatasetName);
fprintf(fId2, '\nDataset is %s\n',DatasetName);
	...
	for it=1:maxIteration
		...
		P_rel=zeros(ma.nInstances,N_hum*ha.nClasses);
		  ...
		  consensus_Labels(consensus_Labels==-1)=0;
                  ...
                  disp(size(LRep));
                  disp(size(P_rel));
                  rel_New = findKappaUserLabel(P_rel, LRep);          % kappa values for each user , label
                  Kappa_New = findKappaUserLabel(P_rel, trueRep);          % kappa values for each user , label
