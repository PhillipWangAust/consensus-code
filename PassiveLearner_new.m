...
for i=1:expNum
	...
	ha=HumanAnnotator_scene(DatasetName,N_hum,1-test_fraction,0);
	%disp(size(trainLabel,1));
	...
	for j=1:N_mach
		...
		disp(size(lLabel{j}));
		fprintf(fId,size(lLabel{j}));
		fprintf(fId,size(oc_human.A));
		fprintf(fId,size(oc_human.Q));
		fprintf(fId,size(oc_human.DnInv));
		fprintf(fId,size(ma.labelled_train_data));
