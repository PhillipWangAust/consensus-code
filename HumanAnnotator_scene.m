classdef HumanAnnotator_scene< handle
    ...
    methods
	function obj = HumanAnnotator_scene(Datasetname,nMod,fraction,flipProbability)
		...
		%% Partition data for training and testing
                data=dlmread('./scene/WTfeatures_500.txt');
		...
		%test_label=labels(temp==0,:);
			for i=1:obj.nModels
				A=dlmread(strcat('./scene/user', num2str(i), '.csv'));
				annotatorLabels(i,:,:)=A(:,:);
       		end
		%celldisp(centroid);
