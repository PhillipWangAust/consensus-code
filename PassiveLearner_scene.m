...
Folder2='../../Output_new/';
fId = fopen(strcat(Folder2, 'PassiveLearnerBatch_', DatasetName, '.txt'), 'w');
fId2 = fopen(strcat(Folder2, 'PassiveLearnerBatchfScore_', DatasetName, '.txt'), 'w');
fId3 = fopen(strcat(Folder2, 'PassiveLearnerBatchconfusion_', DatasetName, '.txt'), 'w');
fId4 = fopen(strcat(Folder2, 'PassiveLearnerBatchrunningtime_', DatasetName, '.txt'), 'w');
fprintf(fId, '\n-----------------------------------------------------\n');
fprintf(fId2, '\n-----------------------------------------------------\n');
fprintf(fId, '\nDataset is %s\n',DatasetName);
fprintf(fId2, '\nDataset is %s\n',DatasetName);
...
for i=1:expNum
	...
	ha=HumanAnnotator_scene(DatasetName,N_hum,1,0);
	%disp(size(trainLabel,1));
	fprintf(fId,'before machine models');
	%ha=initialize(Folder1,DatasetName,N_hum,i);
	ma=MachineAnnotator(ha,N_mach,test_fraction,initialtrainingSize);
	fprintf(fId,'after machine models');
	oc_human=ma.oc_obj;
	fprintf(fId,'oc_human initial matrix sizes');
	disp('oc_human initial matrix sizes');
	disp('oc_human initial matrix sizes');

	disp(size(oc_human.A));
	disp(size(oc_human.Q));
	disp(size(oc_human.DnInv));
	disp(size(ma.labelled_train_data));
	%disp(size(trainLabel,1));
	for j=1:N_mach
        	temp_machine= squeeze(ma.annotator(j,:,:));
        	lLabel{j}=temp_machine;
		disp(size(lLabel{j}));
        	fprintf(fId,'done with sizes');
	...
	dlmwrite('train_data.txt', ma.temp);
	for it=1:maxIteration
		...
		for count=1:5
			dlmwrite(strcat(Folder2, 'PassiveLearnerBatch_', DatasetName, '.txt'), ma.temp_labelled, '-append');
			unlabelled_index=instanceMatrix(xor(ma.temp_labelled,ma.temp));
			dlmwrite(strcat(Folder2, 'PassiveLearnerBatch_', DatasetName, '.txt'), unlabelled_index, '-append');
			instanceId = randsample(unlabelled_index,1);
			fprintf(fId,'selected instanceid before if is %d\n',instanceId);			
			%disp(size(selectedInstanceList));
			%disp(unlabelledCount);
