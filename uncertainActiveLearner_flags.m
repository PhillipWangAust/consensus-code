...
%basic variables
N_hum = 8;                     % N_human human models are created
...
fId5 = fopen(strcat(Folder2, 'uncertainActiveBatchReliability_', DatasetName, '_20.txt'), 'w');
fId6 = fopen(strcat(Folder2, 'uncertainActiveBatchKappaConsensus_', DatasetName, '_20.txt'), 'w');
fId7 = fopen(strcat(Folder2, 'uncertainActiveBatchKappaTrueLabels_', DatasetName, '_20.txt'), 'w');
fprintf(fId, '\n-----------------------------------------------------\n');
fprintf(fId2, '\n-----------------------------------------------------\n');
fprintf(fId, '\nDataset is %s\n',DatasetName);
fprintf(fId2, '\nDataset is %s\n',DatasetName);
...
for i=1:expNum
	...
	fprintf(fId6,'expnum is %d\n',i);
	fprintf(fId7,'expnum is %d\n',i);
	%disp(size(trainLabel,1));
	ha=HumanAnnotator_flags(DatasetName,N_hum,1,0.25);
	%disp(size(trainLabel,1));
	fprintf('before machine models');
	rel=ones(1,N_hum*ha.nClasses);
	%ha=initialize(Folder1,DatasetName,N_hum,i);
	ma=MachineAnnotator_flags(ha,N_mach,test_fraction,initialtrainingSize);
	...
	for it=1:maxIteration
		...
		instancesAdded=0;
		m=0;
		P_rel=zeros(ma.nInstances,N_hum*ha.nClasses);
                m_re=0;
                for re=1:N_hum
                        m_re=m_re+1;
                        P_rel(:,(m_re-1)*ha.nClasses+1:m_re * ha.nClasses)=squeeze(ha.annotator(re,ma.labelled_train_index,:));

                end
		  trueLabels=ha.train_label(ma.labelled_train_index,:);
		  disp(consensus_Labels);
		  consensus_Labels(consensus_Labels==-1)=0;
		  disp(consensus_Labels);
	          disp(P_rel(:,1:7));
		  disp(findKappaUserLabel(P_rel(:,1:7),consensus_Labels));
		  %disp(LRep);
                  LRep = repmat(consensus_Labels, 1,N_hum);
		  disp(LRep);
		  disp(P_rel);
		  trueRep=repmat(trueLabels,1,N_hum);
                  disp((LRep));
                  disp((P_rel));
                  rel_New = findKappaUserLabel(P_rel, LRep);          % kappa values for each user , label 1,-1
                  Kappa_New=findKappaUserLabel(P_rel,trueRep); % labels 1,0
		  Kappa_truth=findKappaUserLabel(consensus_Labels,trueLabels);
		  Kappa_New=[Kappa_New,Kappa_truth]; 
		  rel = rel + gamma * (rel_New- rel);
                  fprintf(fId5,'%f ',rel);
		  fprintf(fId5,'\n');
                  fprintf(fId6,'%f ',rel_New);
		  fprintf(fId6,'\n');
                  fprintf(fId7,'%f ',Kappa_New);
		  fprintf(fId7,'\n');
	...
	% predictions for unlabelled data
		...
		for count=1:5
			...
			incrementalAvector=ha.getVfromX(instanceId,5,L_temp,oc_human.A);
			...
			ma.labelled_train_index=[ma.labelled_train_index;instanceId];
                        ma.nInstances=ma.nInstances+1;
                        consensus_Labels=[consensus_Labels;selectedInstanceLabels(1,:)];
