...
for expNum=1:experiments
	...
	%fprintf(fId,'flipProbability in mimicing humans is %f\n',flip);
	ha=HumanAnnotator(Dataset,N_human,fraction,flip);	
        OL = ha.train_label;
 	%fprintf(fId,'no of training instances is %d\n',size(ha.train_data,1));
	%fprintf(fId,'no. of testinstances is %d\n',size(ha.test_label,1));
        %for j=1:N_human
